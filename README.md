...
To run a **_Parallel Hill Climber_**, run the command:

```bash
$ python3 search.py phc
```

To run the **_(μ,λ) Evolutionary Algorithm_**, run the command:

```bash
$ python3 search.py ml
```

To run **_Age-Fitness Pareto Optimization_**, run the command:
...
**2 minute version:** A longer video showing a randomly generated creature vs. creatures evolved using each of these evolutionary algorithms can be seen [here](https://youtu.be/_7sgRzGf3dw).

**B Roll:** Some additional evolved creatures can be viewed [here](https://youtu.be/bHlpQYCeKPE).
